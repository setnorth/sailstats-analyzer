...
const FILE_FIELD_NAMES: [&str;13] = ["awa",
                                    "aws",
                                    "latitude",
                                    "longitude",
                                    "hdg",
...
#[tauri::command]
async fn get_slices( fields: Vec<String>, 
              start: usize, 
              stop: usize, 
              data: State<'_,Data>) -> Result<Vec<Vec<Point>>, String> {
  ...
  for field in fields {
    let src_vec : &Vec<Point> = db.get(&field).ok_or_else(|| {format!("Could not get field {}.",field)})?;
...
#[tauri::command]
async fn get_radars(fields: Vec<String>,
                        start: usize, 
                        stop: usize, 
                        data: State<'_,Data>) -> Result<Vec<Vec<f32>>, String> {
  let db = data.lock().or(Err("Could not lock db"))?;
  ...
  for field in fields{
    ...
    if mode[0] == "avg" {
      let quantities : &Vec<Point> = db.get(mode[1]).ok_or_else(|| {format!("Table {} not found.",mode[1])})?;
      let directions : &Vec<Point> = db.get(mode[2]).ok_or_else(|| {format!("Table {} not found.",mode[2])})?;
      ...
      for (value,direction) in quantities[start..stop].iter().zip(directions[start..stop].iter()){
        let direction = direction.y.floor() as usize % 360;
        let value = value.y;
        fres[direction] = (fres[direction]*fcount[direction]+value)/(fcount[direction]+1.0);
        fcount[direction] += 1.0;
      }
    //Calculate the percentage a specific angle is measured
    }else if mode[0] == "dist" {
      let mut occurences : &[Point] = &db.get(mode[1]).ok_or_else(|| {format!("Table {} not found.", mode[1])})?;
      //Vector might be shorter due to filtered top 1% values
...
fn get_path(start: usize, stop: usize, data: State<Data>) -> Result<Vec<[f32;2]>, &str>{
  ...
  let lats = db.get("latitude").ok_or("Could not find field 'latitude'.")?;
  let longs = db.get("longitude").ok_or("Could not find field 'longitude'.")?;
...
async fn read_file(file_name: String, data: State<'_,Data>) -> Result<(), String>{
  ...
  for field in OUTLIER_FIELDS{
    let vec : &mut Vec<Point> = db.get_mut(field)
                                  .ok_or_else(|| {format!("Could not get field {}.",field)})?;
    let max : f32 = vec.iter()
                        .map(|p| p.y)
                        .reduce(f32::max)
  ...
  for (field, magnitude) in low_pass_fields{
    let v = low_pass(&db.get(field).ok_or_else(|| {format!("Could not get {}", field)})?.to_vec(), magnitude);
...
fn derive_values(db: &mut HashMap<String,Vec<Point>>) -> Result<(), String>{
  ...
  for ((((p_awa, p_aws), p_hdg), p_cog), p_sog) in awa_vec.iter()
                                                          .zip(aws_vec)
                                                          .zip(hdg_vec)
                                                          .zip(cog_vec)
                                                          .zip(sog_vec){
    ...
    let awd = if (hdg + awa) < 360.0 { hdg + awa } else { hdg + awa - 360.0 };
    ...
    let twa = if (twd - hdg) >= 0.0 { twd - hdg} else { twd - hdg + 360.0 };

    let time = &p_awa.x;

    awd_vec.push(Point{x: time.clone(), y: awd});
    tws_vec.push(Point{x: time.clone(), y: tws});
    twd_vec.push(Point{x: time.clone(), y: twd});
    twa_vec.push(Point{x: time.clone(), y: twa});
  }
...
fn low_pass(raw_vec: &[Point], wsize: usize) -> Vec<Point>{
  ...
  lp_vec.push(Point{ x: raw_vec[wsize/2].x.to_string(),
                     y: raw_vec[..wsize].iter()
                                        .map(|p| p.y)
                                        .sum::<f32>() / wsize as f32 });
  ...
  for i in wsize/2+1..raw_vec.len()-wsize/2{
    ...
    lp_vec.push(Point{x: raw_vec[i].x.to_string(),
                      y: lp_vec[i-1].y + (raw_vec[i+wsize/2].y - raw_vec[i-wsize/2].y) / wsize as f32});
  }
...
#[tauri::command]
async fn get_stats(fields: Vec<String>, start: usize, stop: usize, data: State<'_,Data>) -> Result<HashMap<String,(f32,f32,f32)>, String>{
  let mut res : HashMap<String,(f32,f32,f32)> = HashMap::new();
  let db = data.lock().or(Err("Could not lock data."))?;
  for field in fields{
    let stop = cmp::min(stop, db.get(&field)
                                  .ok_or_else(|| {format!("Could not get field '{}'.",field)})?
                                  .len());
